...
const questions = [
  ...
  {
    id: 'comprometimento',
    title: 'Quanto você está disposto a se comprometer com um plano validado para melhorar sua situação financeira?',
    subtitle: 'Seja honesto sobre sua motivação',
    type: 'single',
    options: [
      { value: 'pouco', label: 'Pouco', icon: '🤔', description: 'Vou experimentar' },
      { value: 'medio', label: 'Médio', icon: '💪', description: 'Estou motivado' },
      { value: 'muito', label: 'Muito', icon: '🔥', description: 'Vou me dedicar' },
      { value: 'sangue_nos_olhos', label: 'Sangue nos olhos de vontade', icon: '⚡', description: 'Estou determinado a mudar' },
    ],
  },
  {
...
export default function QuizForm({ onComplete }) {
  ...
  const handleSelect = (questionId, value) => {
    ...
    if (step < questions.length - 1) {
      ...
      // É a última pergunta - finalizar quiz imediatamente
      setTimeout(() => {
